...
export default function NewPurchaseOrderPage() {
  ...
  const isBudgetEnough = (estimationPrice: number) => {
    if (estimationPrice > selectorPO.budget) {
      return false;
    }
    return true;
  }
            ...
            <div className='flex gap-5'>
              {
                selectorPO.checkBudget && (
                  <div className='flex flex-col gap-3'>
                    ...
                    {
                      selectorPO.checkBudget.total && (
                        <p>Rp.  {selectorPO.checkBudget.total.toLocaleString('id-ID')}</p>
                      )
                    }
                  </div>
                )
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            {formData.items.map((item: any, index: number) => (
              <div key={index} className="flex flex-col gap-3">
                ...
                {/* <div className="flex flex-row gap-3">
                  <p className="text-black font-bold">Harga Item :</p>
                  <input type="number" name={"price-" + index} className="bg-white text-black border border-black" value={item.price} onChange={handleItemChange}/>
                </div> */}
                <div className="flex flex-row gap-3">
                  <label className="text-black">Quantity</label>
                  <input
