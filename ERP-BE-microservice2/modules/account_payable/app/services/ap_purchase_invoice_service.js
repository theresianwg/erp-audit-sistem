const apPurchaseInvoiceRepository = require('../repositories/ap_purchase_invoice_repository');
...
exports.createInvoice = async (req) => {
    let data = req.body;
    if (!data.id || (await apPurchaseInvoiceRepository.findById(data.id))) {
        data.id = await generateApPurchaseInvoiceId();
    }

    data = await processInvoiceData(data);
    return await apPurchaseInvoiceRepository.create(data) && await glJournalService.createJournalandJournalDetailThroughPurchaceInvoice(data.taxes_included)
};

exports.findAll = async () => {
    return await apPurchaseInvoiceRepository.findAll();
};

exports.findById = async (id) => {
    return await apPurchaseInvoiceRepository.findById(id);
};

exports.findLatest = async () => {
    return await apPurchaseInvoiceRepository.findLatest();
};

exports.update = async (id, invoiceData) => {
    return await apPurchaseInvoiceRepository.update(id, invoiceData);
};

exports.delete = async (id) => {
    return await apPurchaseInvoiceRepository.delete(id);
};
...
exports.searchInvoices = async (req) => {
    ...
    return await apPurchaseInvoiceRepository.searchInvoices(searchParams);
};
...
exports.generateCSV = async () => {
    ...
    const formattedInvoices = invoices.map(formatInvoiceForCSV);
    return await generateCSVPurchaseInvoice(formattedInvoices);
};

exports.updatePaymentStatus = async (id, paymentStatus) => {
    return await apPurchaseInvoiceRepository.updatePaymentStatus(
        id,
        paymentStatus,
    );
};
...
exports.updateSelectedFields = async (id, invoiceData) => {
    ...
    return await apPurchaseInvoiceRepository.updateSelectedFields(
        id,
        invoiceData,
    );
};
